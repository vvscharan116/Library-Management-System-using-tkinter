...
def viewm():
    ...
    Label(labelFrame, text="%-20s%-40s%-30s%-20s" % ('Roll', 'Name', 'Gender', 'Member Type'), bg='black', fg='white').place(
        relx=0.07, rely=0.1)
    ...
    try:
        ...
        con.commit()
        for i in cur:
            Label(labelFrame, text="%-20s%-40s%-30s%-20s" % (i[1], i[0], i[2], i[3]), bg='black', fg='white').place(
                relx=0.07, rely=y)
            y += 0.1
    except:
